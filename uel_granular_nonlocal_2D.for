...
!  nonlocal granular flow under two-dimensional, generalized
...
!  and 'generalized' axisymmetric settings. The setting is 'generalized' 
!  in the sense that out-of-plane deformation (theta-direction) may 
!  be non-zero. However, d/dtheta of all quantities remains zero, 
!  enabling a two-dimensional description.
!
! Solution variables (or nodal variables) are the r- and z-components
!  of displacement (DOFs 1-2) the out-of-plane angular motion (DOF 5)
...
!
! In the input file, the element type of the dummy mesh should be CGAX4.
!
!************************************************************************
...
!****************************************************************************
     ...
     +  adlmag(mdload,*),predef(2,npredf,nnode),ddlmag(mdload,*),period
      ...
      !
      integer i,j,k,l,jj,A11,A12,B11,B12,nInt,nIntPt,intpt,nDim,nCoord,
     +  ngSdv,nlSdv,stat,matflag
      !
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !
      parameter(nDim=3)  ! number of spatial dimensions, do not change
      parameter(nCoord=2)! number of coordinate dimensions, do not change
     ...
     +  dsh0(nNode,2),dshC0(nNode,2),detMapJ0,detMapJ0C,R0ref,R0,
     +  F0_tau(3,3),detF0_tau,xi(nInt,2),w(nInt),dshxi(nNode,2),
     +  Fp_t(3,3),sh(nNode),dsh(nNode,2),dshC(nNode,2),detMapJ,
     +  detMapJC,Rref,R,F_tau(3,3),detF_tau,v_tau(3),v_mag,g_tau,
     +  dgdx(2,1),T_tau(3,3),gr(1,1),Fp_tau(3,3),mu_tau,taubar_tau,
     +  press,gammadotp_tau,Ctang(3,3,3,3),Lambda(3,3),Gamma(3,3),
     +  dgrdg(1,1),Bmat(6,3*nNode),Smat(6,1),Nmatg(1,nNode),
     +  Gmatg(2,nNode),Gmat(9,3*nNode),Gmat0(9,3*nNode),Cmat(9,9),
     +  Qmat(9,9),LambdaMat(9,1),GammaMat(1,9),Qmatg(1,9),Atang(3,3,3),
     +  Amat(2,9)
      !
     ...
     +     Pi=3.141592653d0,three=3.d0,third=1.d0/3.d0)
      ...
      ! Obtain the referential and current radial coordinate 
      ...
      do i=1,nNode
         ...
         R0ref = R0ref + sh0(i)*coords(1,i)
         ! radial coord in current config at centroid
         R0 = R0 + sh0(i)*(coords(1,i) + u(i,1))
      enddo
      ...
      !
      F0_tau(3,1) = R0*F0_tau(3,1)
      F0_tau(3,2) = R0*F0_tau(3,2)
      F0_tau(3,3) = R0/R0ref ! 'hoop' stretch
      !
      ! Get the volume ratio at the centroid
      !
      ...
      do intpt=1,nIntPt
         ...
         ! Obtain the referential and current radial coordinate at 
         ...
         do i=1,nNode
            ...
            Rref = Rref + sh(i)*coords(1,i)
            ! radial coord in current config
            R = R + sh(i)*(coords(1,i) + u(i,1))
         enddo
         ...
         ! Modify where appropriate
         !
         F_tau(3,1) = R*F_tau(3,1)
         F_tau(3,2) = R*F_tau(3,2)
         F_tau(3,3) = R/Rref ! 'hoop' stretch
         !
         ...
         !
         v_mag = dsqrt(v_tau(1)**two + v_tau(2)**two + 
     +                 (R*v_tau(3))**two)
     ...
     +                 gammadotp_tau,Ctang,Lambda,Gamma,dgrdg,stat)
         ...
         globalSdv(elemPt,intPt,5) = v_tau(1)      ! velocity - r-component
         globalSdv(elemPt,intPt,6) = v_tau(2)      ! velocity - z-component
         globalSdv(elemPt,intPt,7) = v_tau(3)      ! angular velocity - theta-component
         ...
         do k=1,nNode
            ...
            Bmat(3,1+nDim*(k-1)) = sh(k)/R
            Bmat(4,3+nDim*(k-1)) = R*dshC(k,2)
            Bmat(5,3+nDim*(k-1)) = R*dshC(k,1)
            Bmat(6,1+nDim*(k-1)) = dshC(k,2)
            Bmat(6,2+nDim*(k-1)) = dshC(k,1)
         enddo
         !
         ...
         !
         Ru = Ru - detmapJC*w(intpt)*(two*pi*R)*
     +                matmul(transpose(Bmat),Smat)
         ...
         !
         Rg = Rg + detMapJC*w(intpt)*(two*pi*R)*
     +     (matmul(transpose(Gmatg),dgdx)+matmul(transpose(Nmatg),gr))
         ...
         do k=1,nNode
            ...
            Gmat(3,3+nDim*(k-1)) = R*dshC(k,1)
            Gmat(4,1+nDim*(k-1)) = dshC(k,2)
            Gmat(5,2+nDim*(k-1)) = dshC(k,2)
            Gmat(6,3+nDim*(k-1)) = R*dshC(k,2)
            Gmat(9,1+nDim*(k-1)) = sh(k)/R
         enddo
         !
         Gmat0 = zero
         do k=1,nNode
            ...
            Gmat0(3,3+nDim*(k-1)) = R0*dshC0(k,1)
            Gmat0(4,1+nDim*(k-1)) = dshC0(k,2)
            Gmat0(5,2+nDim*(k-1)) = dshC0(k,2)
            Gmat0(6,3+nDim*(k-1)) = R0*dshC0(k,2)
            Gmat0(9,1+nDim*(k-1)) = sh0(k)/R0
         enddo
         !
     ...
     +        - (two/three)*T_tau(3,3)
         ...
         !
         Kuu = Kuu + detMapJC*w(intpt)*(two*pi*R)*
     +              (
     ...
     +              )
         ...
         !  tangent matrix.
         !
         Kug = Kug + detMapJC*w(intPt)*(two*pi*R)*
     +                (
     ...
     +                   dgdx(j,1)*iden(k,l))
         ...
         !  to the tangent matrix.
         !
         Kgu = Kgu - detMapJC*w(intPt)*(two*pi*R)*
     +              (
     ...
     +              )
         ...
         !
         Kgg = Kgg - detMapJC*w(intPt)*(two*pi*R)*
     +               (
...
!************************************************************************
      ...
      parameter(zero=0.d0,one=1.d0,fourth=0.25d0)
